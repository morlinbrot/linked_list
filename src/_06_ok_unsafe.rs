...
impl<T> List<T> {
    ...
    // we only need to advance the tail pointer forward when pushing.
    ...
    pub fn pop(&mut self) -> Option<T> {
        // SAFETY: `as_mut` is safe since we are not handing out the reference to anywhere.
        // SAFETY: All allocations are cleaned up by reconstructing the `Box` before returning.
        unsafe {
    ...
    pub fn peek(&self) -> Option<&T> {
        // SAFETY: Aliasing rules are upheld by tying the lifetime of the data to self.
...
mod test {
    ...
    fn miri_food() {
        ...
        // Drop it on the ground and let the destructor exercise itself
    }
}
